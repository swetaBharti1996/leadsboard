...
export const loadPosts = (queryType) => (dispatch, getState) => {
  ...
  axios.get(`http://localhost:8888/website/scrapper/facebook/getPosts?type=${queryType}`, tokenConfig(getState))
    .then(res => {
      console.log(res);
      dispatch({
        type: POST_LOADED,
...
export const loadEmails = (postUrl) => (dispatch, getState) => {
  ...
  axios.post(`http://localhost:8888/website/scrapper/facebook/getEmails`, body, tokenConfig(getState))
    .then(res => {
      dispatch({
        type: EMAILS_LOADED,
        payload: res.data
